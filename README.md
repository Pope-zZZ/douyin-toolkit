...
上游 `yzfly/douyin-mcp-server` 是给 MCP 客户端调用的命令行服务，本仓库在它基础上做了下面这些。

**新增功能**
- **本地网页界面**（FastAPI，`http://localhost:8080`）：贴链接、点按钮就能用，不用配 MCP、不用敲命令；带 `/api/health` 供程序探活
- **主页批量**：给一个用户主页链接（或 `sec_uid`），批量「列出 / 下载 / 提取文案」，可限数量和单条时长
- **多链接批量提取**：多个链接一行一个，页面自动出现「批量提取」
- **视频下载入口**：网页上直接下载（下载文件名做了编码处理，中文标题不会乱码）；命令行 `--action download` 出 mp4
- **油管支持**：取信息 / 字幕 / 下载，可选画质（最高 / 4K / 2K / 1080p / 720p / 480p / 仅音频），支持代理与 cookies；没有字幕时回落到语音识别
- **免安装打包**：PyInstaller 打包成 one-folder 版，内置 ffmpeg / ffprobe，双击 exe 自动开浏览器，零安装
- **提取结果落盘**：文案存成文件，提取时可选 `--save-video` 一并保存视频
- **只提取前 N 秒**：长视频先取一段，省时间也省识别额度

**对原有功能的改动**
- **下载链路**：从分享页解析无水印地址（`playwm` → `play`），并加上 Session 预热与多次重试，链接失效自动重来
- **Key 机制**：不再用环境变量硬塞，改成每位用户首次在网页里配自己的硅基流动 Key，工具不内置任何 Key
- **启动方式**：`launcher` 自动把内置 ffmpeg 加进 PATH、起服务、开浏览器
- **模板渲染兼容**：同时兼容新旧 Starlette 的 `TemplateResponse` 调用方式（新版下首页不再 500）
