...
上游 `yzfly/douyin-mcp-server` 已经带了命令行脚本和一个本地网页版（抖音下载＋文案提取）。本仓库在它基础上做了下面这些。

**新增功能（原项目没有）**
- **YouTube 支持**：取视频信息 / 字幕 / 下载，可选画质（最高 / 4K / 2K / 1080p / 720p / 480p / 仅音频），支持代理与 cookies；没有字幕时回落到语音识别（`src/web/youtube.py` ＋ `/api/youtube/*`）
- **主页批量**：给一个用户主页链接（或 `sec_uid`），批量「列出 / 下载 / 提取文案」，可限数量和单条时长（`--max-count` / `--max-duration`）
...
**在原项目基础上改进（原本就有，我改了）**
- **网页界面**：网页版是原项目自带的，我在它上面改的（前端模板、批量入口、下载交互、接口补充）
